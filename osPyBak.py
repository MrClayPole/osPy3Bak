...
def prune_os_snapshots(vm_name, os_images, os_snapshots, os_snapshot_prefix, os_snapshot_date, os_snapshot_retention):
    for os_snapshot in os_snapshots:
        if os_snapshot.name.startswith("snapshot for " + vm_name + os_snapshot_prefix):
            if datetime.date(os_snapshot_date) - timedelta(days=os_snapshot_retention) >= datetime.strptime(os_snapshot.name[-10:], "%Y-%m-%d").date():
                get_cinder_interface(args).volume_snapshots.delete(os_snapshot.id)
                print (" DELETED: ", os_snapshot.name)

def prune_os_images(vm_name, os_images, os_snapshot_prefix, os_snapshot_date, os_snapshot_retention):
    for os_image in os_images:
        if os_image.name.startswith(vm_name + os_snapshot_prefix):
            if datetime.date(os_snapshot_date) - timedelta(days=os_snapshot_retention) >= datetime.strptime(os_image.name[-10:], "%Y-%m-%d").date():
                get_glance_interface(args).images.delete(os_image.id)
                print (" DELETED: Image", os_image.name)

def get_nova_interface(args):
    return nova_client.Client(2.1, session=set_session(args) )

def get_cinder_interface(args):
    return cinder_client.Client(3.59, session=set_session(args) )

def get_glance_interface(args):
    return glance_client.Client(2, session=set_session(args) )

def get_keystone_interface(args): 
    return keystone_client.Client (interface='public', session=set_session(args) )
...
os_projects = get_keystone_interface(args).projects.list()
...
for os_project in os_projects:
    ...
    os_images = get_nova_interface(args).glance.list()
    os_snapshots = get_cinder_interface(args).volume_snapshots.list()
    os_vms = get_nova_interface(args).servers.list()
    for os_vm in os_vms:
        ...
        if os_snapshot_retention > 0:
            try:
                get_nova_interface(args).servers.create_image(os_vm.id, os_vm.name + os_snapshot_prefix + os_snapshot_date.strftime("%Y-%m-%d"))
            ...
            success += 1
        prune_os_images(os_vm.name, os_images, os_snapshot_prefix, os_snapshot_date, os_snapshot_retention)
        prune_os_snapshots(os_vm.name, os_images, os_snapshots, os_snapshot_prefix, os_snapshot_date, os_snapshot_retention)
