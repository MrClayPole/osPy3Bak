...
for os_project in os_projects:
    ...
    os_volume = get_cinder_interface(args).volumes.list()
    os_volume_attachments = get_cinder_interface(args).attachments.list()
    os_snapshots = get_cinder_interface(args).volume_snapshots.list()
    os_quotas = (get_cinder_interface(args).quotas.get(os_project.id))
    os_snapshot_count = 0
    ...
    for os_volume_attachment in os_volume_attachments:
        ...
        os_ospybak_gb_per_snapshot = os_ospybak_gb_per_snapshot + get_cinder_interface(args).volumes.get(os_volume_attachment.volume_id).size
